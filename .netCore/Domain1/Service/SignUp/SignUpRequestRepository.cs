...
namespace Domain.Service.SignUp
{
    public class SignUpRequestRepository : ISignUpRequestRepository
    {
        ...
        public async Task addResumeToProfile(Guid profileId, Guid resumeId, Guid jobSeekerId, string profileName, string profileSummary)
        {
            ...
            var existingProfile = await _context.JobSeekerProfiles.FindAsync(profileId);
            if (existingProfile == null)
                throw new Exception("Profile not found");

            existingProfile.ResumeId = resumeId;
            existingProfile.ProfileName = profileName;
            existingProfile.ProfileSummary = profileSummary;
            existingProfile.JobSeekerId = jobSeekerId;
