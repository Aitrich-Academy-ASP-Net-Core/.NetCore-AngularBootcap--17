...
namespace Domain.Service.Authuser
{
    public class AuthUserRepository : IAuthUserRepository
    {
        ...
        public AuthUserRepository(AppDbContext context, IConfiguration config)
        {
            _context = context;
            _config = config;
        }
        ...
        public string CreateToken(AuthUser user)
        {
            ...
            var key = new SymmetricSecurityKey(
                Encoding.UTF8.GetBytes(_config["AuthSettings:Token"])
            );
        ...
        {
            var user = await GetAuthUserByUserEmail(email);
            if (user == null || string.IsNullOrEmpty(user.PasswordHash)) return null; // ✅ null safe
...
//                authUser.PasswordHash = PasswordHelper.HashPassword(authUser.Password);
...
//    new Claim(ClaimTypes.Email, user.Email),
//    new Claim(ClaimTypes.Sid, user.Id.ToString()),
//    new Claim(ClaimTypes.Role, "ADMIN")  // ⚡ use ClaimTypes.Role here
//};
...
//        //==============JOBSEEKER TOKEN=================
//        public string CreateJobSeekerToken(AuthUser jobSeeker)
//        {
//            var claims = new List<Claim>
//    {
//        new Claim(ClaimTypes.Name, jobSeeker.FirstName),
//        new Claim(ClaimTypes.Email, jobSeeker.Email),
//        new Claim(ClaimTypes.Sid, jobSeeker.Id.ToString()),
//        new Claim(ClaimTypes.Role, "JOB_SEEKER") // Always JOB_SEEKER
//    };
